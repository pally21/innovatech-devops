...
| **Monitoreo** | CloudWatch recibe métricas de CPU/memoria de las tareas ECS (usadas por la política de autoescalado, ver sección 6). El envío de **logs** de aplicación a CloudWatch Logs se intentó configurar pero está bloqueado por permisos de `LabRole` (ver sección 11); como alternativa dentro del laboratorio se usa `docker logs <container>` en la instancia EC2. |
...
| Las tareas con `logConfiguration: awslogs` nunca pasaban de `PENDING` (`is missing an attribute required by your task`) | Diagnóstico paso a paso: (1) se verificó `ECS_AVAILABLE_LOGGING_DRIVERS` en `/etc/ecs/ecs.config` — no persistía entre reinicios del agente vía `--env-file`; (2) tras pasar la variable directo con `-e` al `docker run`, el atributo `com.amazonaws.ecs.capability.logging-driver.awslogs` sí apareció en la Container Instance; (3) la tarea **seguía** fallando, esta vez exigiendo `ecs.capability.execution-role-awslogs`; (4) se simuló el permiso con `aws iam simulate-principal-policy --policy-source-arn LabRole --action-names logs:CreateLogStream logs:PutLogEvents`, devolviendo **`implicitDeny`** en ambas acciones. | Causa raíz confirmada: **`LabRole` no incluye permisos de CloudWatch Logs** (`logs:CreateLogStream`, `logs:PutLogEvents`) entre sus políticas adjuntas, y AWS Academy no permite crear ni modificar roles IAM para añadirlos. Se revirtió a la Task Definition sin `logConfiguration` (revisión estable) y se documentó `docker logs <container>` en la instancia EC2 como la vía de acceso a logs disponible dentro de las restricciones del laboratorio. |
